...
struct GameView2Revisit: View {
    ...
    var body: some View {
        VStack {
            Gauge(value: Float(gameData.revisitationCurrentIndex ?? -1), in: 0.0 ... Float(gameData.revisitationQuestions.count)) {
                Text("interval \(1 + (gameData.revisitationCurrentIndex ?? -1))")
            } //ga
            Button("  ") {
